...
function Navbar() {
    ...
    const {item, isCart} = useContext(CartContext)
    console.log(item)
                ...
                <div className='flex gap-4 md:gap-10 items-center'>
                    <div onClick={cartOpen} type='button'>
                        <div className='relative'>
                            ...
                            {isCart && <span className="cartbtn--badge">{item}</span>}
                        </div>
                        {!cartDrawer && <div className='absolute w-[21.5rem] right-4 md:right-20 shadow-xl top-20  md:top-28 md:w-[22rem] rounded-lg z-20 h-[250px] bg-white'>
                            ...
                            <hr />
                            <div className='flex px-6 py-3 text-Darkgrayishblue justify-center  items-center'>
                                <div className='flex items-center gap-4 font-kumbh '>
                                    {!isCart ?
                                    (
                                        <div className='flex h-full items-center justify-center pt-14'>Your cart is empty</div>
                                    ):
                                    (<>
                                        <img src={thumbnail} className='rounded-lg w-14' />
                                        <ul className='text-sm'>
                                            <li>Fall Limited Edition Sneakers</li>
                                            <li>$125.00 x {item} <span className='font-bold text-slate-900'>{125 * item}.00</span> </li>
                                        </ul>
                            ...
                            {isCart && <div className='px-6'><button className='mt-3 w-full rounded-lg text-white font-bold hover:bg-orange-300 bg-Orange py-3'>Checkout</button></div>}
                        </div> }
                    </div>
