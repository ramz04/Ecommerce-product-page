...
function LightBoxOpen(){
  setLightbox(prevlbox => !prevlbox)
}
const addToCart = () => {
    return count > 0 ? setCart(!cart) : false
}

  return (
    <>
      <CartContext.Provider value={{item: count, isCart: cart}}>
        <Navbar />
      </CartContext.Provider>
      <div className='home--page md:px-[10rem] md:grid md:grid-cols-2 md:mt-14'>
        ...
        <div className='h-full hidden md:flex flex-col items-center'>
          <div onClick={LightBoxOpen} className='cursor-pointer'><img src={data[currentIndex].url} className='rounded-2xl w-[450px]'/></div>
          <ul className='flex items-center justify-between gap-8 mt-3'>
            {imageCarousel}
          </ul>
          <Lightbox 
            lbox={!lightbox}
            closelbox={lbox => setLightbox(lbox => !lbox)}
        ...
        <div className='main--section p-6 md:p-[3.3rem] font-kumbh'>
          <h3 className='font-kumbh uppercase tracking-wide md:tracking-wider font-bold text-Orange'>sneaker company</h3>
          <h1 className='font-bold mt-2 text-Verydarkblue md:tracking-wide md:text-5xl md:mt-6 text-3xl'>Fall Limited Edition Sneakers</h1>
          <p className='mt-3 text-sm md:mt-6 md:text-base leading-5 text-Darkgrayishblue md:tracking-wider'>These low-profile sneakers are your perfect casual wear companion. Featuring a 
    durable rubber outer sole, they’ll withstand everything the weather can offer.</p>
        <div className='flex md:flex-col md:gap-2 md:items-start justify-between mt-6 items-center '>
          <div className='flex items-center gap-2 md:gap-5'>
            <h2 className='text-3xl md:text-4xl font-bold'>$125.00</h2>
            <div className='text-Orange md:text-base px-2 text-sm font-bold py-1 rounded-lg bg-Paleorange '>50%</div>
          </div>
          <p className='font-bold md:mt-2 text-Grayishblue line-through'>$250.00</p>
        </div>
        <div className='md:flex md:gap-4'>
          <div className='mt-6 w-full md:basis-2/5 bg-Lightgrayishblue flex justify-between items-center rounded-lg p-4'>
          ...
          <div type='button' onClick={addToCart}  className='w-full md:basis-3/5  flex items-center justify-center mt-4 py-3 md:py-2 rounded-lg bg-Orange gap-4 hover:bg-opacity-75 transition-shadow ease-in-out'>
